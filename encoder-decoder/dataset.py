        ...
        for code, examples in zip(batch_code, batch_example):
            ...
            try:
                ...
                for state in examples:
                    try:
                        env = Grid2D(config.world_height)
                        for i in range(config.world_height):
                            for j in range(config.world_height):
                                ...
                                shape = None
                                if state[i][j][0] == 1:
                                    env._arm_x, env._arm_x = i,j
                                if state[i][j][1] == 1:         
                                    if state([i][j][2] ==1):
                                        removable = True
                                        for c in range(len(config.colors)):
                                            if state[i][j][3+c] == 1:
                                                color = config.colors[c]
                                        for s in range(len(config.shapes)):
                                            if state[i][j][3+len(config.colors)+s] == 1:
                                                color = config.shapes[s] 
                                        env.set_item(color,shape,[i,j])

                        execute(code,env,config.colors, config.shapes)
                        output = env.to_tensor(config.colors, config.shapes)
